...
// ══════════════════════════════════════════════════════════════
const CACHE_VERSION = 'v39.6';        // Erhöht für SW-Update
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(keys =>
      Promise.all(
        keys
          .filter(key => key !== CACHE_STATIC && key !== CACHE_API && key !== CACHE_TILES)
          .map(key => {
...
async function limitTileCacheSize() {
  ...
  if (keys.length > MAX_TILE_CACHE_ITEMS) {
    ...
    console.log(`[SW] Alte Kacheln gelöscht, Cache-Größe auf ${MAX_TILE_CACHE_ITEMS} begrenzt.`);
  }
}
  ...
  if (url.hostname.includes('openfreemap.org')) {
    event.respondWith(
      caches.open(CACHE_TILES).then(async cache => {
        ...
        return new Response('Offline – Kachel nicht verfügbar', { status: 404 });
      })
    );
    return;
  }
