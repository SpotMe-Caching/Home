...
const ORS_KEY =
  "eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjI5NmQwYWMxYmM0MzQxNjdiOTE2Mjk2ZjFiOTk3MmRjIiwiaCI6Im11cm11cjY0In0=";
...
function openNavOverlay(lat, lng, name) {
  ...
  setTimeout(() => {
    ...
    naviMap.on("load", () => {
      ...
      sEl.style.cssText =
        "width:22px;height:22px;background:#ff9f00;border-radius:50%;border:2.5px solid rgba(255,255,255,0.85);box-shadow:0 0 0 4px rgba(255,159,0,0.2),0 0 20px rgba(255,159,0,0.8);";
    ...
    if (navigator.geolocation) {
      naviWatchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (naviUserMarker)
            naviUserMarker.setLngLat([userPos.lng, userPos.lat]);
          ...
          const moved = naviLastRoutePos
            ? haversineKm(
                naviLastRoutePos.lat,
                naviLastRoutePos.lng,
                userPos.lat,
                userPos.lng,
              ) * 1000
            : 999;
...
function naviAddUserMarker() {
  ...
  uEl.style.cssText =
    "width:18px;height:18px;background:#22d3ee;border-radius:50%;border:3px solid white;box-shadow:0 0 10px rgba(34,211,238,0.9);";
...
function naviUpdateDistDisplay() {
  ...
  const m =
    haversineKm(userPos.lat, userPos.lng, naviTarget.lat, naviTarget.lng) *
    1000;
  document.getElementById("naviDistVal").textContent =
...
function naviClearRouteLayers() {
  if (!naviMap) return;
  ["navi-foot-casing", "navi-foot", "navi-car-casing", "navi-car"].forEach(
    (id) => {
...
async function naviFetchRoute() {
  ...
  const bikePromise = fetch(
    `https://router.project-osrm.org/route/v1/bike/${osrmBase}`,
  )
    .then((r) => r.json())
    .catch(() => null);
  ...
  const footPromise = ORS_KEY
    ? fetch(
        `https://api.openrouteservice.org/v2/directions/foot-hiking?api_key=${ORS_KEY}&start=${start}&end=${end}`,
      )
        .then((r) => r.json())
        .catch(() => null)
  ...
  if (footGeometry) {
    ...
    if (footDuration)
      document.getElementById("naviFootVal").textContent = fmtMin(footDuration);
  }
}
